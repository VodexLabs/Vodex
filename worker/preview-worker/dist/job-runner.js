...
async function downloadSourceZip(snapshotPath) {
    ...
    if (error || !data)
        throw new Error(`Source download failed: ${error?.message ?? "missing"}`);
...
async function loadSourceFiles(job) {
...
async function applyProjectMetadata(job, diagnostics) {
    const { data: project } = await supabase
        .from("projects")
        .select("metadata")
        .eq("id", job.project_id)
        .maybeSingle();
    const prev = project?.metadata && typeof project.metadata === "object" && !Array.isArray(project.metadata)
        ? project.metadata
        : {};
...
export async function runJob(job) {
    ...
    try {
        ...
        if (!result.ok) {
            ...
            const buildMeta = previewBuildMeta;
            const errorCode = buildMeta && typeof buildMeta.errorCode === "string" ? buildMeta.errorCode : null;
            const userMessage = buildMeta && typeof buildMeta.userMessage === "string" ? buildMeta.userMessage : null;
            await finishJob(job, {
                status: "failed",
                ...
                diagnostics: {
                    ...
                    previewBuildMeta,
                    packageRepairDiagnostics: buildMeta && typeof buildMeta.packageRepair === "object"
