...
function mapFiles(files) {
    ...
    for (const f of files)
        m.set(norm(f.path), { path: norm(f.path), content: f.content });
    return m;
}
function upsert(m, path, content, actions, reason) {
    const p = norm(path);
...
`;
function detectNext(files) {
    return (files.has("app/page.tsx") ||
        files.has("app/page.jsx") ||
        [...files.keys()].some((p) => p.startsWith("app/") && p.endsWith("page.tsx")));
}
function detectVite(files) {
    return (files.has("vite.config.ts") ||
        files.has("vite.config.js") ||
        files.has("index.html") ||
        files.has("src/main.tsx"));
}
export function runWorkerZipAutoRepair(files) {
    ...
    if (isNext) {
        ...
        if (!m.has("vite.config.ts") && !m.has("vite.config.js")) {
            upsert(m, "vite.config.ts", `import { defineConfig } from 'vite';\nimport react from '@vitejs/plugin-react';\nexport default defineConfig({ plugins: [react()] });\n`, actions, "worker: vite.config.ts");
        }
        if (!m.has("index.html")) {
            upsert(m, "index.html", '<!DOCTYPE html><html><body><div id="root"></div><script type="module" src="/src/main.tsx"></script></body></html>', actions, "worker: index.html");
        }
    }
