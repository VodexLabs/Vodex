-- Admin OTP confirmations + diagnostic logs
...
create index if not exists dreamos_diagnostic_logs_category_idx
  on public.dreamos_diagnostic_logs (category, created_at desc);

create index if not exists dreamos_diagnostic_logs_user_idx
  on public.dreamos_diagnostic_logs (user_id, created_at desc);
