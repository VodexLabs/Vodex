...
export async function listAdminUsers(options: {
  ...
  for (const au of slice) {
    ...
    rows.push({
      ...
      build_credits_cap: Math.max(
        creditCap(buildPlanAllowance, explicitBuildBonus),
        canonicalFields.tokens_remaining,
      ),
      ...
      action_credits_cap: Math.max(
        creditCap(actionPlanAllowance, explicitActionBonus),
        canonicalFields.action_credits_remaining,
      ),
