...
export function MessageDesignIcon({
  ...
  const Icon = ICON_MAP[preset] ?? Sparkles;
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, ${meta.color}, color-mix(in srgb, ${meta.color} 55%, #0f172a))`,
      }}
    >
      <Icon className={size === "sm" ? "size-4" : "size-4"} strokeWidth={1.85} />
    </div>
  );
}
